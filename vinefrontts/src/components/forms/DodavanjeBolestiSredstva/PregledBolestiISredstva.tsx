...
const PregledBolestiISredstva: FunctionComponent = () => {
  ...
  const columns = [
    {
      ...
      render: (text: any, record: any) =>
        TableFieldForModal(text, () => openBolestiModal(record)),
    },
    {
  ...
  const dohvatiPodatkeZaTablicu = () => {
    ...
    if (sresdstvoSwitch) {
      bfsService.getBolestSredstvo(data).then((response) => {
      ...
      bfsService.getBolestFaza(data).then((response) => {
  ...
  const getSveBolesti = () => {
    bolestService.getAllBolestiForCascader().then((response) => {
      setBolesti(response.data);
    });
  };

  const getSvaSredstva = () => {
    sredstvoService.getAllSredstvaForCascader().then((response) => {
      setSredstva(response.data);
    });
  };
  const getSveFaze = () => {
    fazaService.getAllFazeForCascader().then((response) => {
  ...
  const useFilter = () => {
    if (
      ...
      if (sresdstvoSwitch) {
        bfsService.getBolestSredstvoFiltered(data).then((response) => {
          setPageNo(response.data.pageable.pageNumber);
          setTotalItems(response.data.totalPages);
          setTableDataSredstva(response.data.content);
        });
        ...
        bfsService.getBolestFazaFiltered(data).then((response) => {
          setPageNo(response.data.pageable.pageNumber);
          setTotalItems(response.data.totalPages);
          setTableDataFaze(response.data.content);
        });
  ...
  return (
    <div>
      <div className="splitter-div">
        <div className="left-div">
          <p className="form-label">
            {constants.BOLESTI_SREDSTVA_SWITCH_SREDSTVA}
          </p>
          <Switch
            className="switch-class"
            checked={sresdstvoSwitch}
            onChange={handleTableChange}
          />
          <p className="form-label">
            {constants.BOLESTI_SREDSTVA_BOLEST_CASCADE}
          </p>
          <Cascader
            style={{ height: "20px" }}
      ...
      <h1 className="form-title">
        {constants.BOLESTI_SREDSTVA_TABLICA_SREDSTVA_NASLOV}
      </h1>
      <Table
      ...
      />
      <BolestModal
        visibleModal={bolestiModalVisible}
        closeModal={closeBolestiModal}
        bolest={bolestiModalName}
      />
      <FazaModal
        visibleModal={fazaModalVisible}
        closeModal={closeFazaModal}
        faza={fazaModalName}
      />
      <SredstvoModal
        visibleModal={sredstvoModalVisible}
        closeModal={closeSredstvoModal}
        sredstvo={sredstvoModalName}
      />
    </div>
  );
};
