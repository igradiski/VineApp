...
const MojVinograd: FunctionComponent = () => {
  ...
  const promijeniStranicuLoza = (page: number, size: number | undefined) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any, record: any) =>
        TableUpdateDelete(
          () => editVinogradClick(text, record),
          () => deleteVinogradClick(record),
  ...
  const getInitialData = async () => {
    ...
    vinogradService.getVinogradi(data).then((response) => {
  ...
  const getVinogradLozaData = async () => {
    const data2: IVinogradLozaFilter = {
      pageNo: lozaPageNo,
      size: size,
      sort: [],
    ...
    if (vinogradId !== "") {
      vinogradLozaService.findByVinogradPaged(data).then((response) => {
    ...
    <div className="tip-sredstva-main-div">
      ...
      {detaljiVisible ? (
        <div style={{ paddingTop: "3%" }}>
          ...
          <Pagination
            pageSize={size}
            total={lozaTotalItems}
            onChange={promijeniStranicuLoza}
          />
        </div>
      ) : (
        ""
      ...
      <InsertVinogradLozaModal
        isVisible={modalLozaVisible}
        closeModal={closeModal}
        vinogradId={vinogradId}
        vinogradName={vinogradName}
      />
      <UpdateVinogradLozaModal
        closeModal={closeModal}
        isVisible={lozaIsUpdateVisible}
        id={updateId}
      />
    </div>
  );
